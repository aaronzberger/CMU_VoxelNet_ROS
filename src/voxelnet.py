...
class FCN(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.linear = nn.parallel.DataParallel(self.linear, (0, 1))

        self.batch_norm = nn.BatchNorm1d(out_channels)
        self.batch_norm = nn.parallel.DataParallel(self.batch_norm, (0, 1))

        self.relu = nn.ReLU()
        self.relu = nn.parallel.DataParallel(self.relu, (0, 1))
...
class VFE(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        concat = torch.mul(concat, mask.type(torch.float32))
...
class SVFE(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(x.shape)
        # maxes = torch.max(x, dim=2, keepdim=True)[0]
        # print(maxes.shape)
        # mask = torch.ne(maxes, 0)
        # print(mask.shape)
...
class Deconv2D(nn.Module):
    def __init__(self, in_channels, out_channels, k, s, p):
        ...
        self.deconv = nn.parallel.DataParallel(self.deconv, (0, 1))
